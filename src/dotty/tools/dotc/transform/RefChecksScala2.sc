...
/*t*/    override def transformInfo(sym: Symbol, tp: Type): Type = {
/*t*/      // !!! This is a sketchy way to do things.
/*t*/      // It would be better to replace the module symbol with a method symbol
/*t*/      // rather than creating this module/method hybrid which must be special
/*t*/      // cased all over the place. Look for the call sites which use(d) some
/*t*/      // variation of "isMethod && !isModule", which to an observer looks like
/*t*/      // a nonsensical condition. (It is now "isModuleNotMethod".)
/*t*/      if (sym.isModule && !sym.isStatic) {
/*t*/        sym setFlag lateMETHOD | STABLE
/*t*/        // Note that this as far as we can see it works equally well
/*t*/        // to set the METHOD flag here and dump lateMETHOD, but it does
/*t*/        // mean that under separate compilation the typer will see
/*t*/        // modules as methods (albeit stable ones with singleton types.)
/*t*/        // So for now lateMETHOD lives while we try to convince ourselves
/*t*/        // we can live without it or deliver that info some other way.
/*t*/        log(s"Stabilizing module method for ${sym.fullLocationString}")
/*t*/      }
/*t*/      super.transformInfo(sym, tp)
/*t*/    }

/*t*/    val toJavaRepeatedParam  = new SubstSymMap(RepeatedParamClass -> JavaRepeatedParamClass)
/*t*/    val toScalaRepeatedParam = new SubstSymMap(JavaRepeatedParamClass -> RepeatedParamClass)

         //// use HasFlags.flagString (scala2) or FlagSet.toString(dotty) instead
...
/* */              }
/* */            }
/* */          } //end of !typesOnly, and also end of checkOverride(SymbolPair)
